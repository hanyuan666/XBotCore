...
XBot::XBotCoreThread::XBotCoreThread ( std::string config_yaml,
    ...
    if ( !std::dynamic_pointer_cast<XBot::HALThread> ( __hal ) ) {
        ...
        std::string path_to_shared_lib = XBot::Utils::FindLib ( lib, "LD_LIBRARY_PATH" );
        if ( path_to_shared_lib == "" ) {
            throw std::runtime_error ( lib + " path must be listed inside LD_LIBRARY_PATH" );
        }
        HALInterface::Ptr joint = SoLib::getFactoryWithArgs<HALInterface> ( path_to_shared_lib, "JOINT",__hal );
        if ( joint ) {
    ...
    if ( hal_lib["IEndEffectors"] ) {
        ...
        for ( const auto& end_eff : list ) {
            ...
            HALInterface::Ptr hand = SoLib::getFactoryWithArgs<HALInterface> ( path_to_shared_lib,"HAND"+iend_eff,halInterface );
    ...
    if ( hal_lib["ISensors"] ) {
        ...
        for ( const auto& sensor : list ) {
            ...
            HALInterface::Ptr sensorptr = SoLib::getFactoryWithArgs<HALInterface> ( path_to_shared_lib,"SENSOR"+isensor,halInterface );
    ...
    if ( options.xbotcore_simulator_mode ) {
        ...
        lib =  "libXBotGazeboRosFT.so";
         /* Obtain full path to shared lib */
        std::string path_to_shared_lib = XBot::Utils::FindLib ( lib, "LD_LIBRARY_PATH" );
        if ( path_to_shared_lib == "" ) {
            throw std::runtime_error ( lib + " path must be listed inside LD_LIBRARY_PATH" );
        }
        HALInterface::Ptr sensorptrft = SoLib::getFactoryWithArgs<HALInterface> ( path_to_shared_lib,"SENSORft",halInterface );
        if ( !halInterface->isLoaded ( "ft" ) ) halInterface->addLib ( "ft",sensorptrft );
        
        lib =  "libXBotGazeboRosIMU.so";
         /* Obtain full path to shared lib */
        path_to_shared_lib = XBot::Utils::FindLib ( lib, "LD_LIBRARY_PATH" );
        if ( path_to_shared_lib == "" ) {
            throw std::runtime_error ( lib + " path must be listed inside LD_LIBRARY_PATH" );
        }
        HALInterface::Ptr sensorptrimu = SoLib::getFactoryWithArgs<HALInterface> ( path_to_shared_lib,"SENSORimu",halInterface );
